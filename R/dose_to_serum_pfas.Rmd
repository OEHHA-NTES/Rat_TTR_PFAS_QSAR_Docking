...
model_compare_metrics <- serum_comparison %>%
  left_join(model_compare_metrics %>% dplyr::select(chem, species_name, sex, route, model_used, better_model), by = c("chem", "species_name", "sex", "route", "model_used")) %>% 
      filter(dose_letter != "Zero") %>%
      mutate(
        pred_best = case_when(
