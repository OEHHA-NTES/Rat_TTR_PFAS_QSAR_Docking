...
Note that included GenX and TFA here for demo purposes (to export to
RShiny app)
...
Heather has assembled a large document with many different combinations
of chemicals/sex/species/exposure routes that she needs model params
for.
...
Just investigate unique chems and data availability in EASE-SUITE (for
any species)
...
tk_pfas_wide <- tk_pfas_df %>% 
  filter(standard_endpoint %in% c("CL_app", "CL_ren", "CL",
                                  ...
                                  "kabs",
                                  #two_phase params (alpha and beta model)
                                  "VD2",
                                  "VDc",
                                  "k_alpha",
                                  "k_beta"
...
The biphasic model for PFAAs that are known to follow such kinetic
profile (e.g., GenX, PFOA, PFBS, PFOS) is as follows
(doi.org/10.1016/j.envint.2018.01.011): \### Equation (2): α Phase
...
Where: - $F$: Bioavailable fraction (unitless) - $D$: Administered dose
($\mu g / \text{kg-bw/day}$) - $U$: Unit conversion factor
($24 \, \text{h/day}$) - $Vd_0$: Volume of distribution at time 0
($\text{ml/kg}$) - $k_a$: Absorption rate ($1/\text{h}$) - $k_d$:
Distribution rate into peripheral tissues ($1/\text{h}$) -
...
Where: - $C_{\text{serum}, \beta}(t)$: Serum concentration during the β
phase ($\mu g / \text{ml}$) -
$C_{\text{serum}, \alpha}(t_{\alpha,\text{end}})$: Serum concentration
at the end of the α phase ($\mu g / \text{ml}$) - $k_e$: Elimination
rate ($1/\text{h}$) - $t_{\text{end}}$: Time corresponding to the end of
the experiment
...
Example application 28-day continuous exposure regime with a 100
mg/kg/day dose administered once daily:
...
### How many can use two_phase model?
  ...
  mutate(
    ...
    concentration_time_series = list(
      tibble(
        ...
        # two_phase model (if all alpha/beta phase params are available). 
        ...
        map_dbl(time_points, ~ two_phase_function(D_per_dose = D_per_dose,
          ...
          NA_real_ #if all two_phase params are not present, set to NA
        },
...
The PFOS PBPK model developed by Chou & Lin
(2019)$$<https://pubmed.ncbi.nlm.nih.gov/31152982/>$$ is implemented in
the RShiny application in <https://github.com/OEHHA-NTES/PFAS_TK_Shiny>.
This model was used to estimate serum concentrations of PFOS in male
rats for PFOS and is imported below.
...
model_serum_comparison <- serum_comparison %>% 
  ...
  # Plot the two_phase model points in green
...
model_compare_metrics_intermediate <- serum_comparison %>%
  filter(dose_letter != "Zero") %>%
  ...
  summarise(
    ...
    R2_simple_model      = if (sum(!is.na(Measured_serum) & !is.na(simple_model_conc)) > 1)
      R2(Measured_serum, simple_model_conc, na.rm = TRUE) else NA_real_,
    ...
    R2_full_model        = if (sum(!is.na(Measured_serum) & !is.na(full_model_conc)) > 1)
      R2(Measured_serum, full_model_conc, na.rm = TRUE) else NA_real_,
    ...
    R2_two_phase_model   = if (sum(!is.na(Measured_serum) & !is.na(two_phase_conc)) > 1)
      R2(Measured_serum, two_phase_conc, na.rm = TRUE) else NA_real_,
    ...
    R2_pbpk_model        = if (sum(!is.na(Measured_serum) & !is.na(PBPK_model_conc)) > 1)
      R2(Measured_serum, PBPK_model_conc, na.rm = TRUE) else NA_real_,

    n = sum(!is.na(Measured_serum)),
    .groups = "drop"
  ) %>%
  mutate(
    better_model = case_when(
      !is.na(RMSE_simple_model)    & RMSE_simple_model    <= coalesce(RMSE_full_model, Inf) &
        RMSE_simple_model          <= coalesce(RMSE_two_phase_model, Inf) &
        RMSE_simple_model          <= coalesce(RMSE_pbpk_model, Inf) ~ "simple",

      !is.na(RMSE_full_model)      & RMSE_full_model      <= coalesce(RMSE_simple_model, Inf) &
        RMSE_full_model            <= coalesce(RMSE_two_phase_model, Inf) &
        RMSE_full_model            <= coalesce(RMSE_pbpk_model, Inf) ~ "full",

      !is.na(RMSE_two_phase_model) & RMSE_two_phase_model <= coalesce(RMSE_simple_model, Inf) &
        RMSE_two_phase_model       <= coalesce(RMSE_full_model, Inf) &
        RMSE_two_phase_model       <= coalesce(RMSE_pbpk_model, Inf) ~ "two_phase",

      !is.na(RMSE_pbpk_model)      & RMSE_pbpk_model      <= coalesce(RMSE_simple_model, Inf) &
        RMSE_pbpk_model            <= coalesce(RMSE_full_model, Inf) &
        RMSE_pbpk_model            <= coalesce(RMSE_two_phase_model, Inf) ~ "PBPK",
  ...
  # ---- bias computed from RAW rows using better_model ----
model_compare_metrics <- serum_comparison %>%
  left_join(model_compare_metrics_intermediate %>% dplyr::select(chem, species_name, sex, route, model_used, better_model), by = c("chem", "species_name", "sex", "route", "model_used")) %>% 
      filter(dose_letter != "Zero") %>%
      mutate(
        pred_best = case_when(
          better_model == "simple"    ~ simple_model_conc,
          better_model == "full"      ~ full_model_conc,
          better_model == "two_phase" ~ two_phase_conc,
          better_model == "PBPK"      ~ PBPK_model_conc,
          TRUE ~ NA_real_
        )
      ) %>%
      group_by(chem, species_name, sex, route, model_used, better_model) %>%
      summarise(
        bias_additive = mean(pred_best - Measured_serum, na.rm = TRUE),

        # optional: multiplicative bias (geometric mean ratio Pred/Obs)
        gmr_pred_over_obs = exp(mean(log(pred_best / Measured_serum),
                                     na.rm = TRUE)),

        n_bias = sum(!is.na(Measured_serum) & !is.na(pred_best)),
        .groups = "drop"
      ) %>% 
  left_join(model_compare_metrics, by = c("chem", "species_name", "sex", "route", "model_used", "better_model")
  ) %>%
  # your de-dupe patch (keep if still needed)
...
serum_comparison <- serum_comparison %>% 
  ...
  filter(
    case_when(
      ...
      better_model == "two_phase" ~ !is.na(two_phase_conc),      # Keep only full model data
  ...
  mutate(final_concentration = case_when(
    better_model == "simple" ~ simple_model_conc,  # Keep only simple model data
      ...
      better_model == "two_phase" ~ two_phase_conc,      # Keep only two_phase model data
...
### Male-Only serum prediction analysis

#### Predicted vs. Measured Serum SCatterplot for Male rats only

```{r}
band_df <- tibble(x = 10^seq(log10(1e-1), log10(1000), length.out = 400))

lab_01_int_comma <- function(x) {
  ifelse(
    x < 1,
    format(x, trim = TRUE, scientific = FALSE, nsmall = 1),   # 0.1
    comma(x, accuracy = 1)                                     # 1, 10, 100, 1,000
  )
}

serum_comparison_scatter_base_male <- serum_comparison %>%
  filter(Measured_serum > 0,
         concentration > 0,
         sex == "M",
         species_name == "Rat") %>% 
  ...
  geom_errorbarh(aes(xmin = Measured_serum - Measured_serum_sd, 
                    xmax = Measured_serum + Measured_serum_sd), 
                width = 0.1) +
  scale_colour_discrete_c4a_cat("cols4all.area8d", name = "Chemical") +
  
  # Labels and theme
  labs(title = "Measured vs. Predicted Serum Concentrations (Male Rats)",
       x = "Measured Serum Concentration (mg/L)",
       y = "Predicted Serum Concentration (mg/L)") +
  theme_minimal(base_size = 15) +
  ...
  # Log scale for x-axis
   scale_x_log10(limits = c(1e-1, 1000), oob = scales::oob_keep,
                 # labels in format: 1, 10, 100, 1000, etc.
                 breaks = c(0.1, 1, 10, 100, 1000),
                     labels = lab_01_int_comma
                 ) +  
   scale_y_log10(limits = c(1e-1, 1000), oob = scales::oob_keep,
                 breaks = c(0.1, 1, 10, 100, 1000),
                     labels = lab_01_int_comma
                 ) +  # Log scale for y-axis
  # text labels for 1:1 line and 1:3/3:1 lines (angled to match the slope of the lines)
  annotate("text", x = 1.5, y = 1.5, label = "1:1", color = "gray50", size = 4, vjust = -1, angle = 33) +
  annotate("text", x = 3, y = 1/3, label = "3:1", color = "gray50", size = 4, vjust = -1, angle = 33) +
  annotate("text", x = 1, y = 3, label = "1:3", color = "gray50", size = 4, vjust = -1, angle = 33)

serum_comparison_scatter_male <- serum_comparison_scatter_base_male +
  # Add ribbon around 1:1 line with 3-fold variation
    geom_ribbon(
    data = band_df,
    inherit.aes = FALSE,
    aes(x = x, ymin = x/3, ymax = x*3),
    fill = "grey80", alpha = 0.15
  )

ggsave( "../figures/serum_comparison_scatter_male.jpg",
        serum_comparison_scatter_male,
        width = 9, height = 5,
       dpi = 300)
...
pk_params <- twa_df_multiple %>% 
  # make sure only using the best model (joined previously to df based on performance metrics)
    ...
    better_model == "simple" ~ simple_model_conc,  # Keep only simple model data
      better_model == "full" ~ full_model_conc,      # Keep only full model data
    better_model == "two_phase" ~ two_phase_conc#,      # Keep only full model data
      #better_model == "PBPK" ~ PBPK_model_conc
    )
...
```{r}
# prep references for measured serum data
serum_references <- serum_comparison %>% 
  distinct(Study, species_name, chem, sex)
```
...
condensed_measured_table <- serum_comparison %>%
  group_by(Study, chem, species_name, sex, `Serum/plasma`) %>%
  summarize(
    `Exposure Duration (days)` = first(closest_time), # Assuming closest_time represents exposure duration
    `Doses Administered (mg/kg-d)` = paste(unique(Dose_mg_kg_d), collapse = ", "),
    `Measured Serum Concentrations (mg/L)` = paste(
      paste0(Measured_serum, " ± ", Measured_serum_sd),
      collapse = ", "
    ),
    .groups = "drop"
  ) %>%
  ...
  mutate(`Exposure Duration (days)` = (`Exposure Duration (days)` - 24) / 24)

# Create a publication-ready table using flextable
publication_table <- flextable(condensed_measured_table) %>%
  set_header_labels(
    Study = "Study",
    PFAS = "PFAS",
    Species = "Species",
    ...
    `Doses Administered (mg/kg-d)` = "Doses Administered (mg/kg-d)",
    `Measured Serum Concentrations (mg/L)` = "Measured Serum Concentrations (mg/L)"
  ) %>%
  autofit() %>%
  theme_vanilla()
...
### Males-only model performance metrics for just best-fitting model

```{r}
simple_best_model_metrics <- model_compare_metrics %>% 
  filter(species_name == "Rat",
         sex == "M"
         ) %>% 
  mutate(R2 = case_when(
    better_model == "full" ~ R2_full_model,
    better_model == "simple" ~ R2_simple_model,
    better_model == "two_phase" ~ R2_two_phase_model,
    better_model == "PBPK" ~ R2_pbpk_model,
    T ~ NA_real_
  ),
  RMSE = case_when(
    better_model == "full" ~ RMSE_full_model,
    better_model == "simple" ~ RMSE_simple_model,
    better_model == "two_phase" ~ RMSE_two_phase_model,
    better_model == "PBPK" ~ RMSE_pbpk_model,
    T ~ NA_real_
  ),
  MAE = case_when(
    better_model == "full" ~ MAE_full_model,
    better_model == "simple" ~ MAE_simple_model,
    better_model == "two_phase" ~ MAE_two_phase_model,
    better_model == "PBPK" ~ MAE_pbpk_model,
    T ~ NA_real_
  )) %>%
  mutate(`Model Type` = case_when(
    better_model == "full" ~ "Absorption",
    better_model == "simple" ~ "Absorption-independent",
    better_model == "two_phase" ~ "two_phase",
    better_model == "PBPK" ~ "PBPK",
    T ~ NA_character_
  )) %>%
  select(chem, `Model Type`, R2, RMSE, bias_additive, gmr_pred_over_obs) %>%
  rename(`RMSE (mg/L)` = RMSE,
         `Additive Bias (mg/L)` = bias_additive,
         `Multiplicative Bias (unitless ratio)` =  gmr_pred_over_obs) %>%
  arrange(`RMSE (mg/L)`) %>% 
  mutate_if(is.numeric, ~signif(., 4))

write.csv(simple_best_model_metrics, "../data_output/simple_best_model_metrics.csv")

simple_best_model_metrics
```
